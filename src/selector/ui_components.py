...
def create_sheet_dropdown(sheet_manager: SheetManager) -> gr.Dropdown:
    ...
    choices = [f"{s['name']} ({s['domain']}) - {s['rows']} rows × {s['cols']} cols" for s in sheets]
...
def create_domain_badge(context: ContextPropagator) -> str:
    ...
    badge_html = f"""
    <div style="padding: 15px; border-radius: 8px; background: linear-gradient(135deg, {color} 0%, {color}dd 100%);
...
def create_selector_interface(
    filepath: str,
) -> Tuple[gr.Blocks, SheetManager, ContextPropagator]:
    """
    ...
    with gr.Blocks() as interface:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                preview_output = gr.DataFrame(label="👁️ Preview (first 5 rows)", interactive=False, wrap=True)

            with gr.Column(scale=1):
                domain_badge_output = gr.HTML(value=create_domain_badge(context), label="Current Context")
        ...
        def on_preview(dropdown_value, sheet_names_list):
            ...
            idx = [
                ...
                for i, choice in enumerate(
                    [
                        f"{sheets[i]['name']} ({sheets[i]['domain']}) - "
                        f"{sheets[i]['rows']} rows × {sheets[i]['cols']} cols"
                        for i in range(len(sheet_names_list))
                    ]
                )
                if dropdown_value.startswith(sheets[i]["name"])
            ][0]
...
def create_compact_selector(
    filepath: str,
) -> Tuple[gr.Dropdown, gr.DataFrame, gr.HTML, gr.Button, SheetManager, ContextPropagator]:
    """
